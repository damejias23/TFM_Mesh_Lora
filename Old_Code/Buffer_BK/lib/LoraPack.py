...
def pack_AckLora(DEVICE_ID, DEVICE_Resp, UID_msg, Px):
    global _LORA_PKG_FORMAT, _LORA_PKG_ACK_FORMAT
    pkg_ack = struct.pack(_LORA_PKG_ACK_FORMAT, DEVICE_ID, DEVICE_Resp, UID_msg, Px)
    return pkg_ack
...
def unpack_AckLora(recv_ack,  recv_pkg_len):
   ...
   device_id, device_respond, UID_msg, ack_Px = struct.unpack(_LORA_PKG_ACK_FORMAT, recv_ack)
   return device_id, device_respond, UID_msg, ack_Px

def get_IDdest(recv_pkg, recv_pkg_len):
    global _LORA_PKG_FORMAT, _LORA_PKG_ACK_FORMAT
